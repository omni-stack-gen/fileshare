...
int arpping(u_int32_t yiaddr, u_int32_t ip, unsigned char *mac, char *interface)
{
	...
	{
		LOGE("Could not open raw socket\n");
		return -1;
	}

	if (setsockopt(s, SOL_SOCKET, SO_BROADCAST, &optval, sizeof(optval)) == -1)
	{
		LOGE("Could not setsocketopt on raw socket\n");
		close(s);
		return -1;
	}
	...
	while (timeout > 0)
	{
		...
		else if (FD_ISSET(s, &fdset))
		{
			...
			u_int32_t reply_addr;

			memcpy(&reply_addr, arp.sInaddr, sizeof(arp.sInaddr));

			if (arp.operation == htons(ARPOP_REPLY) &&
			    bcmp(arp.tHaddr, mac, 6) == 0 &&
			    /* *((u_int32_t *) arp.sInaddr) == yiaddr */
				reply_addr == yiaddr)
			{
