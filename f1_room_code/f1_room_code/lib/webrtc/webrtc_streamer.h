...
int webrtc_streamer_send_device_discovery_message();
...
int webrtc_streamer_session_reset_seqnumber(char *sessionId,size_t sessionId_len);
/*
通过信令通道发送消息函数
...
/*
    用于网络变化时。快速从连上网络 例如 wifi 网络转到5G网络的时候。系统检测到转换了网络。可以调用该函数，关闭了信令的socket ,重新链接
*/
int webrtc_streamer_network_reset();
/*
    强行关闭信令的socket 使得服务器即时反应到设备离线。该函数使用需要慎重。因为会关闭重连机制。使用了该函数。设备就不会重连，如果需要重连。需要 webrtc_streamer_uninit()
后 再调用初始胡函数 webrtc_streamer_init()
*/
int webrtc_streamer_signal_socket_close();
/*
...
/*
通过信令通道发送消息函数
会话（sessionId）不一定要在会话列表中存在，需要发送对方的ID参数
*/
...
void webrtc_streamer_send_keyframe_request(char *sessionId,size_t sessionId_len);
#ifdef __cplusplus
}
#endif
