...
static int rtp_vp8_pack_input(void *pack, const void *data, int bytes, uint32_t timestamp)
{
	...
	for (vp8_payload_descriptor[0] = 0x10 /*start of partition*/; 0 == r && bytes > 0; ++packer->pkt.rtp.seq)
	{
		...
		packer->pkt.payloadlen = (bytes + N_VP8_HEADER + RTP_FIXED_HEADER) <= packer->size ? bytes :
		...
		r = packer->handler.packet(packer->cbparam, rtp, n + N_VP8_HEADER + packer->pkt.payloadlen, packer->pkt.rtp.timestamp,
		                           0);
