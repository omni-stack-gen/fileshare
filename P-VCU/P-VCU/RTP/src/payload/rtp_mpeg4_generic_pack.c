...
static int rtp_mpeg4_generic_pack_input(void *pack, const void *data, int bytes, uint32_t timestamp)
{
	...
	for (size = bytes; 0 == r && bytes > 0; ++packer->pkt.rtp.seq)
	{
		...
		packer->pkt.payloadlen = (bytes + N_AU_HEADER + RTP_FIXED_HEADER) <= packer->size ? bytes :
		...
		r = packer->handler.packet(packer->cbparam, rtp, n + N_AU_HEADER + packer->pkt.payloadlen, packer->pkt.rtp.timestamp,
		                           0);
