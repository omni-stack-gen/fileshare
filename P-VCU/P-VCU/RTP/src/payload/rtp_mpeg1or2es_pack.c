...
#include "rtp_payload_internal.h"
#include <stdlib.h>
#include <string.h>
#include <assert.h>
#include <errno.h>
...
static void *rtp_mpeg2es_pack_create(int size, uint8_t pt, uint16_t seq, uint32_t ssrc, struct rtp_payload_t *handler,
                                     void *cbparam)
{
	struct rtp_encode_mpeg2es_t *packer;
	packer = calloc(1, sizeof(*packer));
...
static int rtp_mpeg2es_pack_audio(struct rtp_encode_mpeg2es_t *packer, const uint8_t *audio, int bytes)
{
	...
	for (r = offset = 0; 0 == r && bytes > 0; ++packer->pkt.rtp.seq)
	{
		...
		packer->pkt.payloadlen = (bytes + N_MPEG12_HEADER + RTP_FIXED_HEADER) <= packer->size ? bytes :
		                         (packer->size - N_MPEG12_HEADER - RTP_FIXED_HEADER);
		...
		r = packer->handler.packet(packer->cbparam, rtp, n + N_MPEG12_HEADER + packer->pkt.payloadlen,
...
+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
*/
static int rtp_mpeg2es_pack_slice(struct rtp_encode_mpeg2es_t *packer, const uint8_t *video, int bytes,
                                  struct mpeg2_video_header_t *h, int marker)
{
	int r, n;
	uint8_t *rtp;
