...
int rtp_h264_annexb_nalu(const void *h264, int bytes, int (*handler)(void *param, const uint8_t *nalu, int bytes,
                         int last), void *param);
...
static void *rtp_h266_pack_create(int size, uint8_t pt, uint16_t seq, uint32_t ssrc, struct rtp_payload_t *handler,
                                  void *param)
{
...
static int rtp_h266_pack_fu(struct rtp_encode_h266_t *packer, const uint8_t *ptr, int bytes, int mark)
{
	...
	for (fu_header |= FU_START; 0 == r && bytes > 0; ++packer->pkt.rtp.seq)
	{
		...
		r = packer->handler.packet(packer->cbparam, rtp, n + N_FU_HEADER + packer->pkt.payloadlen, packer->pkt.rtp.timestamp,
		                           0);
		packer->handler.free(packer->cbparam, rtp);
