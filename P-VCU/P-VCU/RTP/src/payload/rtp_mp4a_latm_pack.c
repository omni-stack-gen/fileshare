...
static void *rtp_mp4a_latm_pack_create(int size, uint8_t pt, uint16_t seq, uint32_t ssrc, struct rtp_payload_t *handler,
                                       void *cbparam)
{
...
static int rtp_mp4a_latm_pack_input(void *pack, const void *data, int bytes, uint32_t timestamp)
{
	...
	for (; 0 == r && bytes > 0; ++packer->pkt.rtp.seq)
	{
		packer->pkt.payload = ptr;
		packer->pkt.payloadlen = (bytes + len + RTP_FIXED_HEADER) <= packer->size ? bytes :
		                         (packer->size - len - RTP_FIXED_HEADER);
