...
static void *rtp_vp9_pack_create(int size, uint8_t pt, uint16_t seq, uint32_t ssrc, struct rtp_payload_t *handler,
                                 void *cbparam)
{
...
static int rtp_vp9_pack_input(void *pack, const void *data, int bytes, uint32_t timestamp)
{
	...
	for (vp9_payload_descriptor[0] = 0x08 /*Start of a layer frame*/; 0 == r && bytes > 0; ++packer->pkt.rtp.seq)
	{
		...
		packer->pkt.payloadlen = (bytes + N_VP9_HEADER + RTP_FIXED_HEADER) < packer->size ? bytes :
		                         (packer->size - N_VP9_HEADER - RTP_FIXED_HEADER);
		...
		r = packer->handler.packet(packer->cbparam, rtp, n + N_VP9_HEADER + packer->pkt.payloadlen, packer->pkt.rtp.timestamp,
		                           0);
