...
VOID TUYA_IPC_upload_all_status(VOID)
{
    ...
    CHAR_T night_mode[4] = {0};
    CHAR_T * test = night_mode;
    snprintf(night_mode,4,"%d",IPC_APP_get_night_mode());
    ...
    CHAR_T alarm_sensitivity[4] = {0};
    snprintf(alarm_sensitivity,4,"%d",IPC_APP_get_alarm_sensitivity());
    respone_dp_enum(TUYA_DP_ALARM_SENSITIVITY, alarm_sensitivity );
#endif
...
{
    CHAR_T power_mode[4] = {0};
    snprintf(power_mode,4,"%d",IPC_APP_get_power_mode());
    INT_T percent = IPC_APP_get_battery_percent();

    info("current power mode:%s\r\n", power_mode);
    respone_dp_enum(TUYA_DP_POWERMODE, power_mode);
    info("current battery percent:%d\r\n", percent);
...
STATIC BOOL_T check_dp_bool_invalid(IN TY_OBJ_DP_S *p_obj_dp)
{
    ...
    {
        error("error! input is not bool %d \r\n", p_obj_dp->type);
        return -2;
    }
...
{
    int patrol_state = 0;
    //info("---get_patrol_state\n");
    IPC_APP_patrol_state(&patrol_state);
    info("---get_patrol_state:%d\n",patrol_state);
...
{
    info("error! door bell can only trigged by IPC side.\r\n");
...
VOID TUYA_IPC_handle_dp_cmd_objs(IN CONST TY_RECV_OBJ_DP_S *dp_rev)
{
    ...
    INT_T index = 0;
    debug("3333333333333333333333333\n");
    for(index = 0; index < cnt; index++)
    {
        ...
        for(table_idx = 0; table_idx < table_count; table_idx++)
        {
            ...
            {
                info("*********%d*****\n", s_raw_dp_table[table_idx].dp_id);
