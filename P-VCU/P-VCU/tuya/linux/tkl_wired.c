...
static OPERATE_RET tkl_wired_get_gateway(NW_IP_S *ip)
{
    ...
    while(fgets(buf, sizeof(buf), fp)) {
        ...
        if ((dest == 0) && (addr.s_addr != 0)) {
            strncpy(ip->nwgwstr, inet_ntoa(addr), sizeof(ip->nwgwstr));
            get_gw = 1;
            break;
        }
    }
...
OPERATE_RET tkl_wired_get_ip(NW_IP_S *ip)
{
    ...
    strncpy(ip->nwipstr, inet_ntoa(sin->sin_addr), sizeof(ip->nwipstr));
    ...
    strncpy(ip->nwmaskstr, inet_ntoa(sin->sin_addr), sizeof(ip->nwmaskstr));

    /* get gateway */
    if (tkl_wired_get_gateway(ip) < 0)
        goto com_error;

    // printf("############# ip:%s", ip->nwipstr);
    // printf("# mask:%s", ip->nwmaskstr);
    // printf("# gw:%s\n", ip->nwgwstr);
    close(sock);
    return OPRT_OK;
