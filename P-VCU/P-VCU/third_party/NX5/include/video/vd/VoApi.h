...
{
    char Brightness;            // 亮度，取值范围-127 ~ 128
    unsigned short Saturation;  // 饱和度       默认值：0x200
    char HueSin;                // 色度1，取值范围-32 ~ 32
    char HueCos;                // 色度2，取值范围-32 ~ 32     默认值 0x20
    unsigned char Contrast;     // 对比度       默认值 0x10
} VO_LAYER_CM;
...
Param:
    ...
    CM      in      效果参数结构体 默认值
Return: 成功返回1，失败返回0
Others: 注意！该函数只针对NX5芯片平台，其他芯片平台请勿使用
******************************************************************************/
int VO_ChnSetCM(unsigned int Channel, unsigned int Layer, VO_LAYER_CM *CM);

/******************************************************************************
Function: VO_ChnGetCM
Description: 获取视频显示图层的颜色效果
Param:
    Channel in      指定通道
    Layer   in      指定图层
    CM      out     效果参数结构体
...
Others: 注意！该函数只针对NX5芯片平台，其他芯片平台请勿使用
******************************************************************************/
int VO_ChnGetCM(unsigned int Channel, unsigned int Layer, VO_LAYER_CM *CM);

//Return:jpgdata length 失败返回0
int VO_CapJpgData(char** pjpegdata,unsigned int Channel, unsigned int Layer,int w,int h,VDEC_FRAME_S *pvideoframe);

void VO_FreeJpgData(char* pjpegdata);
