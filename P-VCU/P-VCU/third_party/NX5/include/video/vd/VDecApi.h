...
typedef enum _VDEC_ROTATE_E
{
    VDEC_ROTATE_NONE = 0,
    VDEC_ROTATE_90   = 1,
    VDEC_ROTATE_180  = 2,
    VDEC_ROTATE_270  = 3,
    VDEC_ROTATE_VERTICAL  = 4,
    VDEC_ROTATE_STANDARD = 5,
    VDEC_ROTATE_MAX ,
} VDEC_SHOW_ROTATE_E;
...
/******************************************************************************
Function: Vgs_RotateWork_phy2phy
Description: 旋转指定数据90\180\270度（基于物理地址之间的转换）
Param:
    rotate          in      旋转的枚举
    phyAddr         in      源数据的指定输入的物理地址
    width           in      源数据的宽度
    height          in      源数据的长度
    phyAddrOut      out     输出数据的指定输入的物理地址
Return: 成功返回0，失败返回1
Others:
******************************************************************************/
int Vgs_RotateWork_phy2phy(VDEC_SHOW_ROTATE_E rotate, unsigned int phyAddr, unsigned int width,unsigned int height, unsigned int phyAddrOut);
