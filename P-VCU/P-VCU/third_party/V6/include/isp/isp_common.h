...
#define STAT_POS_OF_AF_PLUS(pos_sel) ((pos_sel>>12)&0x7)
#define STAT_POS_BIT_OF_AWB_ADV 6
#define STAT_POS_MASK_OF_AWB_ADV 3

#define STAT_POS_OF_AWB_ADV2(pos_sel) ((pos_sel>>24)&0x7)  // XC01不支持
#define STAT_POS_BIT_OF_AWB_ADV2 24                        // XC01不支持
#define STAT_POS_MASK_OF_AWB_ADV2 7                        // XC01不支持
#define STAT_POS_OF_AWB0(pos_sel)    ((pos_sel>>27)&0x7)   // XC01不支持
...
typedef enum _ISP_OUT_MODE_
{
    ...
    ISP_OUT_TO_ENG  = (1 << 2),// ISP输出到ENG(XC01不适用)
}ISP_OUT_MODE_E;
...
typedef enum _ISP_STITCH_STAT_SEL_
{
    AE_STAT_TYPE   = 0,//AE统计
    AWB_STAT_TYPE  = 1,//AWB统计
}ISP_STITCH_STAT_SEL;

typedef enum _STITCH_STAT_MODE_
{
    STAT_STITCH_OFF  = 0,//统计独立不拼接
    STAT_STITCH_ON   = 1,//统计拼接
}STITCH_STAT_MODE;
...
typedef struct
{
    ...
    FH_BOOL         bBlocking;    //是否阻塞方式获取  | [0~1]
    FH_UINT32       u32TimeoutMs; //阻塞方式的超时时间ms  | [0~0xffffffff]

    FH_VIDEO_FRAME  stFrm;   //获取到的帧数据 | [FH_VIDEO_FRAME]
}ISP_OUT_STREAM_S;

typedef enum _ISP_SEM_TYPE_E_ {
    ISP_WAIT_ISPF_START = 0,    // ISPF开始信号
    ISP_WAIT_ISPB_START = 1,    // ISPB开始信号
    ISP_WAIT_ISPF_END   = 2,    // ISPF完成信号
    ISP_WAIT_ISPB_END   = 3,    // ISPB完成信号
} ISP_SEM_TYPE_E;
...
typedef struct _ISP_STITCH_CFG_S_
{
    ...
    FH_UINT32            au32DevId[3];  // 各个通路的id | [0~3]
    FH_UINT32            u32TimeThreshold; // 时间间隔单位是us，同步拼接模式下，超过该时间间隔会失同步 | [0~0xffffffff]
} ISP_STITCH_CFG_S;

typedef struct _ISP_SND_PIC_SIZE_
{
    FH_UINT16 u16Width;   // 图像的宽度 | [0~0xffff]
    FH_UINT16 u16Height;  // 图像的高度 | [0~0xffff]
}ISP_SND_PIC_SIZE;

typedef struct _ISP_ECDC_INFO_S_
{
    FH_BOOL   bReady;          //状态值 | [0~1]
    FH_UINT32 u32DataBufAddr;  //数据流的地址 | [0~0xffffffff]
    FH_UINT32 u32CheckSum;     //压缩数据crc值,不带压缩时不填 | [0~0xffff]
}ISP_ECDC_INFO_S;

typedef struct _ISP_SRC_INFO_S_
{
    FH_UINT32 u32VicapDevId;   //vicap的设备号。数据来源不需要释放时，填-1 | [0~2,-1]
    FH_UINT64 u64TimeStamp;    //vicap启动帧的时间戳 | [0~0xffffffff]
    FH_UINT64 u64FrameId;      //isp的帧号 | [0~0xffffffff]
    FH_UINT32 u32EcPara;       //压缩参数 | [0~0xffffffff]
    ISP_SND_PIC_SIZE stPic;    //图像信息 | [ISP_SND_PIC_SIZE]
    ISP_ECDC_INFO_S stSfInfo;  //短帧信息 | [ISP_ECDC_INFO_S]
    ISP_ECDC_INFO_S stLfInfo;  //长帧信息 | [ISP_ECDC_INFO_S]
    FH_UINT32 u32IspDevId;     //通道号 | [0~2]
}ISP_SRC_INFO_S_S;

typedef struct _ISP_RAWEC_ADDR_S_S_
{
    ISP_ECDC_INFO_S stEcdcInfo; //数据流信息 | [ISP_ECDC_INFO_S]
}ISP_RAWEC_ADDR_S_S;

typedef FH_UINT32 VI_DEV;

typedef struct _ISP_VICAP_CALLBACK_INFO_
{
    VI_DEV id;                        //设备号 | [0~2]
}ISP_VICAP_CALLBACK_INFO_S;

typedef struct _ISP_SUSPEND_S_
{
    FH_UINT32 u32Reserved;
}ISP_SUSPEND_S;
...
typedef struct __ISP_SNS_COMIF_S__
{
    FH_UINT32 u32Cmd;
    FH_VOID *pData0;
    FH_VOID *pData1;
...
{
    FH_BOOL  bInitStatusFlag;  // 初始化状态标志位,0:正常收敛模式 1:快速收敛模式 | [0x0-0x1]
...
typedef struct _ISP_AWB_STATUS_S_
{
    ...
    FH_UINT16 u16RTarGain;// 红色目标增益（U4.6） | [0x0~0x3ff]
    FH_UINT16 u16GTarGain;// 绿色目标增益（U4.6） | [0x0~0x3ff]
    FH_UINT16 u16BTarGain;// 蓝色目标增益（U4.6） | [0x0~0x3ff]
} ISP_AWB_STATUS;
...
typedef struct _ISP_GAMMA_CFG_S_
{
    ...
    FH_BOOL         bAutoCGammaEn;    // CGamma自动曲线开关，仅当gammaMode=2且eCCurveType=1时有效：0：CGamma仅由CGammaTable确定; 1：CGamma由两条曲线决定，内插模式见autoCGammaMode | [0~1]
    FH_UINT8        u08AutoCGammaMode;    // CGamma自动曲线模式： 0：由CGammaTable决定；1：由YGammaTable决定（复用作为第二条CGamma）; 2：由内插权重cGammaWgt决定; | [0~0xff]
    FH_UINT8        u08CGammaWgtInttMap[4];  // CGamma自动曲线权重 intt map 0(1/16 inttMax时) : 0x40全用CGammaTable， 0xff全用YGammaTable | [0x0~0xff]
    FH_UINT8        u08CGammaWgtMap[12];    // CGamma自动曲线权重gan map 0(0db时) : 0x40全用CGammaTable， 0xff全用YGammaTable | [0x0~0xff]
} ISP_GAMMA_CFG;
/**|BLC|**/
...
/**|LUT|**/
typedef enum  LUT_COMP_MODE_S_{
    LUT_COMP_16BIT = 0x0, //16bit -> 16bit
    LUT_COMP_12BIT = 0x1, //12bit -> 16bit
}LUT_COMP_MODE;

typedef struct _ISP_LUT_CFG_S_
{
    LUT_COMP_MODE eLutMode; // Lut曲线的模式 | [0x0~0x2]
    FH_UINT16 u16NodeX[12]; // Lut曲线的第一～十二个折点横坐标(Xi < X(i+1)),其中第一个点固定设为0 | [0x0~0xffff]
    FH_UINT16 u16NodeY[12]; // Lut曲线的第一～十二个折点纵坐标。 | [0x0~0xffff]
} ISP_LUT_CFG;
...
typedef struct _ISP_LSC_CFG_2_S_
{
    FH_BOOL   bLscEn; // 镜头阴影矫正控制使能 | [0~1]
    FH_UINT8  u08LscSupStr; // 镜头阴影矫正抑制强度,值越大镜头阴影矫正越弱 | [0~255]
    FH_BOOL   bMirrorEn; // lsc表mirror开关使能 | [0~1]
    FH_BOOL   bFlippEn; // lsc表flip开关使能 | [0~1]
    FH_BOOL   bTabMergeEn; // 两张lsc表融合开关 | [0~1]
    ...
    ISP_BAYER_TYPE u08OriginPattern; // 初始bayer数据格式，仅当mirror或flip开关打开时使用 | [0~3]
    ISP_BAYER_TYPE u08InvertPattern; // flip或mirror后的bayer格式，仅当mirror或flip开关打开时使用 | [0~3]
...
typedef struct _ISP_NR3D_CFG_S_
{
    ...
    FH_UINT8 u08CoeffIdxMap[12];//  不同增益时的coeff映射线 | [0x0-0x7]
...
typedef struct _ISP_DRC_CFG_S_
{
    ...
    FH_UINT16 u16Data[386];// DRC_KL_CURVE系数0 | [0x0~0xffff]
    FH_BOOL bAutoCurveEn;// DRC自动曲线开关，drcCurveMode=1时有效 | [0x0-0x1]
    FH_UINT8 u08AutoCurveMode;// DRC自动曲线模式 | [0x0-0x2]
    FH_UINT8 u08DrcCurveWgt;// DRC Curve权重U.6, 0x40表示只用DRC_KL_CURVE，0xff表示只用DRC_KL_CURVE_1, 中间内插 | [0x0-0xff]
    FH_UINT8 u08KLCurveWgtInttMap[4];// DRC Curve权重 intt map 0(1/16 inttMax时) | [0x0~0xff]
    FH_UINT8 u08KLCurveWgtMap[12];// DRC Curve权重 gain map(0db时) | [0x0~0xff]
    FH_UINT16 u16Curve1Data[386];// DRC_KL_CURVE_1系数0 | [0x0~0xffff]
} ISP_DRC_CFG;
/**|CFA|**/
typedef struct _ISP_CFA_CFG_S_
{
    ...
    FH_BOOL bFalColEn;// 去高频伪色使能(XC01不支持) | [0x0~0x1]
    ...
    FH_UINT8 u08FcStr;// 去伪色强度(XC01不支持) | [0x0~0xff]
    ...
    FH_UINT8 u08ConFilterThh;// 对比度校正上阈值，值越小平滑越明显，分辨率保护力度越弱(XC01不支持), conFilterThh > conFilterThl | [0x0~0xff]
    FH_UINT8 u08ConFilterThl;// 对比度校正下阈值，值越小平滑越明显，分辨率保护力度越弱(XC01不支持) | [0x0~0xff]
    FH_UINT16 u16OffsetT;// 纹理信息偏色量(XC01不支持) | [0x0~0xfff]
    FH_UINT16 u16OffsetY;// 亮度信息偏色量(XC01不支持) | [0x0~0xfff]
    ...
    FH_UINT8 u08ConFilterThhMap[12];// 0db亮度校正上阈值(XC01不支持) | [0x0~0xff]
    FH_UINT8 u08ConFilterThlMap[12];// 0db亮度校正下阈值(XC01不支持) | [0x0~0xff]
} ISP_CFA_CFG;
/**|HLR|**/
...
typedef enum _STAT_SEL_MODULE
{
    ...
    STAT_SEL_AWB0    = 5, // 统计类型为AWB0，XC01不支持
    STAT_SEL_AF      = 6, // 统计类型为AF
} STAT_SEL_MODULE;
...
typedef enum _STAT_POS_SEL_AWB_ADV
{
    ...
    POS_SEL_AWB_ADV_AFTER_DG1 = 4, // AWB_ADV统计位置在DG1，XC01不支持
}STAT_POS_SEL_AWB_ADV;
typedef enum _STAT_POS_SEL_AWB0
{
    POS_SEL_AWB0_AFTER_DG0 = 0, // AWB0统计位置在DG0，XC01不支持
    POS_SEL_AWB0_AFTER_DG3 = 1, // AWB0统计位置在DG3，XC01不支持
    POS_SEL_AWB0_AFTER_LUT = 2, // AWB0统计位置在LUT，XC01不支持
    POS_SEL_AWB0_AFTER_LSC = 3, // AWB0统计位置在LSC，XC01不支持
    POS_SEL_AWB0_AFTER_DG1 = 4, // AWB0统计位置在DG1，XC01不支持
}STAT_POS_SEL_AWB0;
...
typedef enum _STAT_POS_SEL_AF_PLUS
{
    POS_SEL_AF_PLUS_AFTER_LFBLC = 0, // AF统计位置在长帧BLC输出
    POS_SEL_AF_PLUS_AFTER_SFBLC = 1, // AF统计位置在短帧BLC输出
    POS_SEL_AF_PLUS_AFTER_LUT   = 2, // AF统计位置在LUT输出
    POS_SEL_AF_PLUS_AFTER_DG2   = 3, // AF统计位置在DG2输出
    POS_SEL_AF_PLUS_AFTER_NR3D  = 4, // AF统计位置在NR3D输出
    POS_SEL_AF_PLUS_AFTER_DRC   = 5, // AF统计位置在DRC输出
    POS_SEL_AF_PLUS_AFTER_LCE_Y = 6, // AF统计位置在LCE_Y输出
    POS_SEL_AF_PLUS_AFTER_YGAMMA= 7, // AF统计位置在YGAMMA输出
}STAT_POS_SEL_AF_PLUS;
...
typedef struct _GLOBE_STAT_S
{
    ...
    FH_UINT64 u64FrmCnt; // 该统计对应的帧号　| [0~0xffffffffffffffff]
} GLOBE_STAT;
...
typedef struct _DRC_INIT_CFG_S_
{
    FH_BOOL bDrcHwEn;// Drc硬件模块使能： 0：关闭； 1：打开； | [0x0~0x1]
    DRC_STAT_CFG stDrcStatCfg;// Drc统计窗口0和窗口1配置 | [DRC_STAT_CFG]
    FH_UINT16 u16LumaDiffTh;// 控制滤波后亮度差，越大亮处抑制越小  | [0x0~0xffff]
...
{
    FH_UINT16 gamma[1024];  // gamma表对应的值 | [0~0x3ff]
} GAMMA_INIT_CFG;
...
typedef struct
{
    FH_UINT64 u64OutStartFrmCnt; // ISP输出的上一帧的帧号 | [0~0xffffffffffffffff]
    FH_UINT64 u64OutEndrmCnt;   // ISP输出的当前真的帧号 | [0~0xffffffffffffffff]
    FH_UINT32 u32RstCnt;        // 复位的次数 | [0~0xffffffff]
    FH_UINT32 u32ViEcErrDropCnt; // vicap ecdc校验错误丢帧的计数
    ISP_PIC_SIZE stInPic;       // 输入的幅面宽高信息 | [ISP_PIC_SIZE]
    ISP_PIC_SIZE stOutPic;      // 输出的幅面宽高信息 | [ISP_PIC_SIZE]
    FH_UINT32 u32Nr3dRawCrc;    // NR3D压缩的图像CRC值，仅表征图像复杂度，不作为唯一判断标准 | [0~0xffffffff]
    FH_UINT8 u8ViFroZen;      // 检测图像是否发生了冻结，开启NR3D的时候有效 | [0~0xffffffff]
    FH_UINT8 u8IspProcessOrderErr; // ISP多级流水处理顺序是否正常，读清 | [0~1]
    FH_UINT8 u8IspMirror;      // ISP是否开启mirror | [0~1]
    FH_UINT8 u8SnsMirror;      // Sensor是否开启mirror | [0~1]
    FH_UINT8 u8SnsFlip;        // Sensor是否开启flip | [0~1]
    FH_UINT8 u8ViFrmLoss;     // VICAP传递过来的帧号不连续 | [0~1]
    FH_UINT8 u8ViEcdcErr;     // vicap ecdc校验错误，离线模式，读清 | [0~1]
    FH_UINT8 u8Nr3dEcdcErr;   // Nr3dEcdc校验错误，读清 | [0~1]
    FH_UINT8 u8ViOvf;          // IspVi发生溢出，读清 | [0~1]
    FH_UINT8 u8IspRst;         // ISP是否发生过复位，读清 | [0~1]
}ISP_DBG_INFO_S;
