 ...
 *
 * @retval  0              成功
 * @retval  "非0"          失败，其值参见 @namelink{错误码,vb_errcode} 。
 *
 * @note
 ...
 * @brief         设置视频缓存池属性
 *
 * @param[in]     pstVbConf          视频缓存池属性指针,静态属性。
 *
 * @retval  0              成功
 * @retval  "非0"          失败，其值参见 @namelink{错误码,vb_errcode} 。
 *
 ...
 * @brief         创建一个视频缓存池。
 *
 * @param[in]     u32BlkSize         缓存池中每个缓存块的大小
 * @param[in]     u32BlkCnt          缓存池中缓存块的个数
 * @param[in]     pcMmzName          缓存池所在 DDR 的名字
 *
 * @retval  0              成功
 * @retval  "非0"          失败，其值参见 @namelink{错误码,vb_errcode} 。
 *
 ...
 * @param[in]     u32BlkCnt          缓存池中缓存块的个数
 * @param[in]     pcMmzName          缓存池所在 DDR 的名字
 * @param[in]     bComm              是否为common VB
 *
 * @retval  0              成功
 * @retval  "非0"          失败，其值参见 @namelink{错误码,vb_errcode} 。
 *
 * @note
 *    - bComm为false时，此接口功能同 #FH_VB_CreatePool ；
 *    - bComm为true时，用于动态创建common pool；
 *    - 此接口创建的common pool可通过 #FH_VB_DestroyPool 销毁。
 *
 */
VB_POOL FH_VB_CreatePoolEx(FH_UINT32 u32BlkSize,FH_UINT32 u32BlkCnt,const FH_CHAR *pcMmzName,FH_BOOL bComm);

/**
 * @brief         销毁一个视频缓存池。
 *
 * @param[in]     Pool               缓存池 ID 号
 *
 * @retval  0              成功
 * @retval  "非0"          失败，其值参见 @namelink{错误码,vb_errcode} 。
 *
 * @note
 *    - 销毁一个不存在的缓存池，则返回 #FH_ERR_VB_UNEXIST 。
 ...
 *    - 需确保Pool为 #FH_VB_CreatePool 所创建的缓存池的ID号，否则会返回失败。
 *
 */
 ...
 *    - 则可以将第1个参数Pool设置为 #VB_INVALID_POOLID ，将第2个参数设为需要的缓存块大小，
 ...
 *
 */
FH_PHYADDR FH_VB_Handle2PhysAddr(VB_BLK Block);

/**
 * @brief         用户态获取一个帧缓存块所在缓存池的 ID
 *
 * @param[in]     Block              缓存块句柄
 *
 * @retval  0              成功
 ...
 *
 */
FH_SINT32 FH_VB_GetBlkVirAddr(VB_POOL Pool, FH_PHYADDR uPhyAddr, FH_VOID **ppVirAddr);


/**
 * @brief         初始化模块公共视频缓存池
 *
 * @param[in]     enVbUid            使用模块公共视频缓冲池的模块 ID
 *
 * @retval  0              成功
 * @retval  "非0"          失败，其值参见 @namelink{错误码,vb_errcode} 。
 *
 ...
 *    - 需先调用 #FH_VB_Init 进行公共视频缓冲池初始化。
 *    - 需先调用 #FH_VB_SetModPoolConf 配置缓存池属性，再初始化，否则会失败。
 *    - 可反复初始化，不返回失败
 ...
 *
 * @param[in]     enVbUid            使用模块公共视频缓冲池的模块 ID
 * @param[in]     pstVbConf          模块公共视频缓存池属性指针，静态属性
 *
 * @retval  0              成功
 * @retval  "非0"          失败，其值参见 @namelink{错误码,vb_errcode} 。
 *
 ...
 *
 * @param[in]     enVbUid            使用模块公共视频缓冲池的模块 ID
 * @param[out]    pstVbConf          模块公共视频缓存池属性指针
 *
 * @retval  0              成功
 * @retval  "非0"          失败，其值参见 @namelink{错误码,vb_errcode} 。
 *
...
/**
 * @brief         仅创建虚拟缓存池，池里没有物理缓存块。
 *
 * @param[in]     u32BlkCnt          缓存池中缓存块的个数
 * @param[in]     pcMmzName          缓存池所在DDR的名字
 *
 * @retval  0              成功
 * @retval  "非0"          失败，其值参见 @namelink{错误码,vb_errcode} 。
 *
 ...
 * @param[in]     Pool               缓存池 ID 号
 * @param[in]     uPhyAddr           缓存块的物理地址
 * @param[in]     u32Size            缓存块大小
 *
 * @retval  0              成功
 * @retval  "非0"          失败，其值参见 @namelink{错误码,vb_errcode} 。
 *
 * @note
 *    - 此缓存块由用户自己分配，不在公共VB池内，此接口只是负责把缓存块加入 VB 池。
 *    - 不能再调用 #FH_VB_GetBlock 获取此缓存块。
 ...
 *    - 用户可以调用 #FH_VB_ReleaseBlock 从缓存池里释放缓存块。释放后缓存块就不再在缓存池里，
 *    - 用户如果需要再使用，需要再次调用 #FH_VB_AddBlock 接口把缓存块增加到缓存池。
 *    - 用户可以调用 #FH_VB_InquireUserCnt 查询此缓存块是否被占用。
 *
 */
VB_BLK FH_VB_AddBlock(VB_POOL Pool , FH_PHYADDR uPhyAddr, FH_UINT32 u32Size);
