...
/*音频硬件suspend,
 * 1) 只能在MCU RT-Thread端执行, 不可在linux端执行
 * 2) 请确保AI/AO都是disable的状态下，才能调用这个函数
 * 3) 可以关闭中断的情况下调用这个函数
 */
FH_SINT32 FH_AC_Suspend(void);
...
/*for AI*/
FH_SINT32 FH_AC_AI_Select_Dmic(int dmic_id);
