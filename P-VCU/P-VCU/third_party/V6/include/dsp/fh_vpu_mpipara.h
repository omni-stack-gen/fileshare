...
typedef struct
{
	FH_UINT32               crop_en;                     //!< 　裁剪使能。取值范围: [0-1]
	FH_AREA                 vpu_crop_area;               //!< 　裁剪区域
}FH_VPU_CROP;

typedef struct
{
	FH_SIZE                 vi_size;      //!<  输入幅面。取值范围: [16-最大处理幅面]
	FH_VPU_CROP             crop_area;    //!<  通道裁剪区域(FH889x不支持)
}FH_VPU_SIZE;

typedef struct
{
	FH_SIZE                 vpu_chn_size; //!<  通道输出幅面。取值范围: [16-最大处理幅面]
	FH_VPU_CROP             crop_area;    //!<  通道裁剪区域(FH889x不支持)
	FH_UINT32               stride;       //!<  通道输出stride(配置为0时为通道幅面宽,以pixel为单位)
	FH_UINT32               offset;       //!<  通道输出地址偏移(以pixel为单位)
	FH_UINT32               depth;        //!<  通道输出队列深度(需要小于通道buffer个数)。取值范围: [1-6]
}FH_VPU_CHN_CONFIG;

typedef struct
{
	FH_VPU_CHN_CONFIG chn_config;
	FH_UINT32         addr_veu_online;
}FH_VPU_CHN_CONFIG_EX;
...
typedef struct
{
	FH_UINT32              masaic_enable; //!<  隐私屏蔽形式 0:单色 1:马赛克。取值范围: [0-1]
	FH_UINT32              masaic_size;   //!<  马赛克单块大小 0:16x16,1:32x32,2:64x64(FH889x不支持64x64)。取值范围: [0-2]
}FH_MASK_MASAIC;

typedef struct
{
	FH_BOOL                 mask_enable[MAX_MASK_AREA]; //!<  隐私屏蔽使能。取值范围: [0-1]
...
typedef struct
{
	FH_UINT32               magic_color_en; //!<  是否使用magic_color,仅在16bit模式下有效。取值范围: [0-1]
	FH_UINT32               magic_color;    //!<  magic_color的值,16bit,此颜色将作为透明。取值范围: [0-0xffff]
	FH_UINT32               global_alpha_en;//!<  全局透明度使能。取值范围: [0-1]
	FH_UINT32               global_alpha;   //!<  全局透明度。取值范围: [0-255]
	FH_UINT32               rgb16_type;     //!<  位宽16模式时颜色格式:1:ARGB0565,2:ARGB1555,3:ARGB4444。取值范围: [1-3]
	FH_UINT32               extmode;        //!<  0为LSB填0，1为MSB复制到LSB。取值范围: [0-1]
	FH_UINT32               dtvmode;        //!<  0为SDTV模式，1为HDTV模式。取值范围: [0-1]
	FH_UINT32               rgbmode;        //!<  0为stdio RGB，1为computer RGB。取值范围: [0-1]
...
typedef struct
{
	...
	FH_UINT32               logo_idx;      //!<  图像叠加索引号。取值范围: [见芯片能力定义]
	FH_UINT32               logo_pixdepth; //!<  图片像素位宽。取值范围: [见芯片能力定义]
...
typedef struct
{
	FH_UINT32 bgm_enable;    //!<  bgm使能,只有通道0时有效。取值范围: [0-1]
	FH_UINT32 cpy_enable;    //!<  cpy使能,只有通道0时有效。取值范围: [0-1]
	FH_UINT32 sad_enable;    //!<  sad使能,只有通道0时有效。取值范围: [0-1]
...
typedef struct
{
	FH_UINT32 ycmean_en;     //!<  ycmean使能。取值范围: [0-1]
...
typedef struct
{
	FH_UINT32 bEnable;          //!<  LDC使能。取值范围: [0-1]
	FH_UINT32 centerX;          //!<  矫正中心水平坐标
	FH_UINT32 centerY;          //!<  矫正中心垂直坐标
	FH_UINT32 distortionRatio;  //!<  LDC矫正比例，单位千分之几。取值范围: [0-1000]
}FH_VPU_LDC;

typedef struct
{
	FH_UINT32 Enable;           //!<  APC使能。取值范围: [0-1]
	FH_UINT32 Mergesel;         //!<  细节增强和边界锐化合并模式。取值范围: [0-1]
	FH_UINT32 PGain;            //!<  总体APC正向增益。取值范围: [0-255]
	FH_UINT32 NGain;            //!<  总体APC负向增益。取值范围: [0-255]
	FH_UINT32 DetailPGain;      //!<  细节增强正向增益。取值范围: [0-255]
	FH_UINT32 DetailNGain;      //!<  细节增强负向增益。取值范围: [0-255]
	FH_UINT32 DetailThL;        //!<  细节增强门限值。取值范围: [0-1023]
	FH_UINT32 DetailThH;        //!<  细节增强门限值。取值范围: [0-1023]
	FH_UINT32 DetailFilterIdx;  //!<  细节增强滤波器索引。取值范围: [0-4]
	FH_UINT32 DetailLutIdx;     //!<  细节增强映射表索引。取值范围: [0-8]
	FH_UINT32 EdgePGain;        //!<  边界锐化正向增益。取值范围: [0-255]
	FH_UINT32 EdgeNGain;        //!<  边界锐化负向增益。取值范围: [0-255]
	FH_UINT32 EdgeThL;          //!<  边界锐化门限值。取值范围: [0-1023]
	FH_UINT32 EdgeThH;          //!<  边界锐化门限值。取值范围: [0-1023]
	FH_UINT32 EdgeFilterIdx;    //!<  边界锐化滤波器索引。取值范围: [0-4]
	FH_UINT32 EdgeLutIdx;       //!<  边界锐化映射表索引。取值范围: [0-8]
}FH_VPU_APC;

typedef struct
{
	FH_UINT32 Enable;    //!<  RGB前处理使能。取值范围: [0-1]
	FH_UINT32 MeansR;    //!<  R通道均值。取值范围: [0-255]
	FH_UINT32 MeansG;    //!<  G通道均值。取值范围: [0-255]
	FH_UINT32 MeansB;    //!<  B通道均值| [0-255]
	FH_UINT32 Gain;      //!<  增益系数。取值范围: [0-255]
}FH_VPU_RGB_PRE;

typedef struct {
	FH_UINT32 mode;           //!<  0:手动模式，用户配置其他参数决定当前相关缩放参数 1:自动模式，驱动基于缩放比自适应旋转缩放参数。取值范围: [0-1]
	FH_UINT32 fix2ds_width;   //!<  fix2ds生效门限，小于此幅面且对应通道输出能力满足时开启，部分通道不支持
	FH_UINT32 scaler_coeff_h; //!<  水平方向滤波器系数，从模糊到清晰。取值范围: [0-15]
	FH_UINT32 scaler_coeff_v; //!<  垂直方向滤波器系数，从模糊到清晰。取值范围: [0-15]
}FH_VPU_SCALER_ATTR;
...
typedef struct
{
	...
	FH_UINT32      vi_auto;        //!<  驱动是否对visize进行自适应更改(使用拼接时要关闭)。取值范围: [0-1]
}FH_VPU_GRP_PARAM;

typedef struct
{
	FH_UINT32      align_patch_mode_w[4];    //!<  宽度四种角度的拉伸模式,在180和270时需要使用0,否则会显示异常　0:拉伸到对齐后幅面 1:拉伸到有效幅面,非对齐部分为冗余图像。取值范围:[0-1]
	FH_UINT32      align_patch_mode_h[4];    //!<  高度四种角度的拉伸模式,在180和90时需要使用0,否则会显示异常　0:拉伸到对齐后幅面 1:拉伸到有效幅面,非对齐部分为冗余图像。取值范围: [0-1]
	FH_UINT32      fcmode;                   //!<  是否进行插帧。取值范围: [0-1]
	FH_UINT32      close_rls_vb;             //!<  是否在关闭通道时释放vb(默认释放)。取值范围: [0-1]
	FH_UINT32      send_real_pts;            //!<  是否在通道帧率控制时发送真实时间戳(默认发送修改后的时间戳)。取值范围: [0-1]
}FH_VPU_CHN_PARAM;
...
typedef struct {
	FH_UINT32 u32Enable;  //!< 通道是否使能
	FH_UINT32 u32Width;   //!< 通道输出宽度
	FH_UINT32 u32Height;  //!< 通道输出高度
	FH_UINT32 u32FPS;     //!< 通道实时帧率，高16位为统计时间，低16位为帧数
	FH_UINT32 u32FrameCnt;  //!< 自通道创建以来的输出帧数

}FH_VPU_CHN_STATUS;

typedef enum {
	VPU_EXTCMD_SET_CALLBACK = 0x76507501,
	VPU_EXTCMD_GET_CHN_STATUS,
	VPU_EXTCMD_CHN_ROTATE_BY_VPPU,  /* 是否使用vppu旋转扩展通道，0：使用vgs(default); 1: vppu */
	VPU_EXTCMD_CHN_SCALE_BY_VPPU,   /* 是否使用vppu缩放扩展通道，0：使用vgs(default); 1: vppu */
	VPU_EXTCMD_BUTT,
}VPU_EXTENSION_CMD_S;
...
}FH_VPU_SET_CALLBACK_PARAM;

typedef FH_SINT32 (*VPU_SEND_CALLBACK)(FH_UINT32 grpidx, void* param);
