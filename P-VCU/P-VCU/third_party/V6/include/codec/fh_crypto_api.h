...
*/
FH_SINT32 FH_CRYPTO_SaveOTPEntry(CRYPTO_HANDLE phCipher, FH_UINT32 u32OptId, FH_UINT8 u8Efuse);
...
/*
功能说明
    set aes lock状态，写1后，在aes的寄存器将看不到用户设置  的key信息。
输入参数
    u8Enable:1-表示使能，看不到key
输出参数
    无；
返回值
    0：函数调用成功。
...
注意
    调用此接口之前，加解密模块必须已初始化。
*/
FH_SINT32 FH_CRYPTO_Lock_AESKey(FH_UINT8 u8Enable);
