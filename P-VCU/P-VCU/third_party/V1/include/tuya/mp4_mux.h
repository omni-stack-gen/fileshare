...
#define swapshort(x) x = (((x & 0xff) << 8) | ((x >> 8) & 0xff))
    ...
    typedef struct
    {
        ...
        unsigned int uiSampleDuration; // in the scale of media's timescale
    } MP4STTSENTRY;
    ...
    typedef struct
    {
        ...
        unsigned int uiSampleDescIndex;
    } MP4STSCENTRY;
    ...
    {
        unsigned int uiSampleCount;
        unsigned int uiSampleOffset; // PTS - DTS
    } MP4CTTSENTRY;

    typedef struct MP4MUXFRAMEINFO {
        unsigned int uiSampleSize;  // stsz, video: 4 byte(file length) + nalu length(without start code)
        long long llPTS;
        long long llDTS;
        int iOffsetInFile;
        int isKeyFrame;
