...
public:
     ...
     * @param chnId the channel opened by openChannel().
     * @param bKeepRender don't stop preview. Display frame buffers must be malloc by viChannel(e.g. rotate), then we
     *   can set true to keep display on screen when stop channel.
     * @return 0 if channel closed successfully, otherwise return error code.
     */
    status_t stopChannel(int chnId, bool bKeepRender = false);
    /**
     ...
     */
    status_t setISPParameters(ISP_DEV nIspDev, CameraParameters &param);
    status_t getISPParameters(ISP_DEV nIspDev, CameraParameters &param);
...
//    status_t OSDOnOff(int chnId, bool bOnOff);
    ...
    status_t getISPDMsg(ISP_DEV nIspDev, int *exp, int *exp_line, int *gain, int *lv_idx, int *color_temp, int *rgain,
        int *bgain, int *grgain, int *gbgain);
