...
typedef struct ISPGeometry
{
    ...
    std::vector<VI_DEV> mScalerOutChns;    //MOD_ID_VIU
}ISPGeometry;
...
struct CameraInfo
{
    ...
    /**
      0: STITCH_NONE
      1: STITCH_2IN1_LINNER
      stitch mode means: two ISPs process one picture, one process left, one process right. last ISP's scalerChn output
      final stitching picture.
      In stitch mode, last scalerChn in mScalerOutChns of last ISPGeometry in mMPPGeometry->mISPGeometrys, is final
      output scaler chn.
    */
    int mStitchMode;
    /**
