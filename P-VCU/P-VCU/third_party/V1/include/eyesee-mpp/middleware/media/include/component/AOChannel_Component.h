...
typedef struct AO_CHN_DATA_S {
    ...
    char                    *pPostBuf; //for postprocess of AudioFrame. don't modify AudioFrame buf content, so we use ours.
    int                     nPostBufSize;
    bool                    bDrainPostDataFlag; //true: need drain and begin to drain! false: not drain.
    BOOL                    mbEnableReSmp;
    //char                    *mpReSmpOutBuf;
    ...
    AOAgc *mpAgcHandle; //agc_handle
    AGC_FLOAT_CONFIG_S mAgcCfg;
    bool bAgcCfgUpdateFlag; //true:update config, false: update done, not update!
    //AgcProcessFuncType pAgcProcessCallback;
    //AgcGetInnerDataInfoFuncType pAgcGetInnerDataInfoCallback;
    ...
    int nStartThresholdTimeMs; //unit:ms
} AO_CHN_DATA_S;
