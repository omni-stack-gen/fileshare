...
typedef struct VIDEOENCDATATYPE {
    ...
    pthread_mutex_t mFlagOutStreamLock;
    ...
    /**
      when pVideoEncData->mEncChnAttr.EncppAttr.eEncppSharpSetting is VencEncppSharp_FollowISPConfig, use it to record
      current venclib encppSharp status.
    */
    BOOL bCurEncppSharpEnable;
    ...
    unsigned char H264NalRefIdcNonZeroValue; //1 or 3. ref to h264 spec, 7.4.1 NAL unit semantics, nal_ref_idc.
    ...
    */
    ...
    int mActiveDropNum; // used in offline encode.
...
//    unsigned int mLastSeiDataConfigLevel;
    VENC_SEI_ATTR stSEIAttr;
    pthread_t SEIThreadId;
    message_queue_t stSEICmdQueue;
    cdx_sem_t stSetSEISem;
