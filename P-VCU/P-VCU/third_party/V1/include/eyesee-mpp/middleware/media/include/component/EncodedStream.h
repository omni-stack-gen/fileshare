...
typedef struct _EncodedStream
{
    ...
    unsigned int   nPackNum;    //one encoded stream may contain several nal units(alsa called pack)
    VencPackInfo   stPackInfos[MAX_OUTPUT_PACK_NUM]; //record every nal unit info: type,offset,length.
