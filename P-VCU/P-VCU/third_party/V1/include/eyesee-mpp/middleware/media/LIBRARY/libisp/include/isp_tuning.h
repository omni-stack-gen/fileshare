...
typedef enum {
	/*inital_cfg_ctrl*/
	ISP_INITIAL_AUTO_N_PRESET_WHITE_BALANCE = 0,
	ISP_INITIAL_WB_MGAIN,
	ISP_INITIAL_EXPOSURE_AUTO,
	ISP_INITIAL_AUTOGAIN,
	ISP_INITIAL_AUTO_EXPOSURE_BIAS,
	ISP_INITIAL_EXPOSURE_ABSOLUTE,
	ISP_INITIAL_ISO_SENSITIVITY,
	ISP_INITIAL_ISO_SENSITIVITY_AUTO,
	ISP_INITIAL_EXPOSURE_METERING,
	ISP_INITIAL_GAIN,
	ISP_INITIAL_POWER_LINE_FREQUENCY,
	ISP_INITIAL_BRIGHTNESS,
	ISP_INITIAL_CONTRAST,
	ISP_INITIAL_SATURATION,
	ISP_INITIAL_SHARPNESS,
	ISP_INITIAL_SCENE_MODE,
	ISP_INITIAL_COLOR_EFFECT,
	ISP_INITIAL_AI_SCENE,
} hw_inital_ctrl_cfg_ids;
...
#if (ISP_VERSION >= 602)
struct isp_awb_light_info_cfg {
	HW_S32		number;
	HW_S32		value[160];
};

struct isp_awb_ext_light_info_cfg {
	HW_S32		number;
	HW_S32		value[80];
};

struct isp_awb_skin_color_info_cfg {
	HW_S32		number;
	HW_S32		value[40];
};

struct isp_awb_special_color_info_cfg {
	HW_S32		number;
	HW_S32		value[80];
};
#else
...
#if (ISP_VERSION >= 602)
	struct isp_awb_light_info_cfg         awb_light_info;         /* id: 0x0200000200 */
	struct isp_awb_ext_light_info_cfg     awb_ext_light_info;     /* id: 0x0200000400 */
	struct isp_awb_skin_color_info_cfg    awb_skin_info;          /* id: 0x0200000800 */
	struct isp_awb_special_color_info_cfg awb_special_info;       /* id: 0x0200001000 */
#else
...
struct isp_tuning_sensor_temp_cfg {
	HW_S16		sensor_temp[14*TEMP_COMP_MAX]; // 14 * number of temp param
};
...
extern vencoder_tuning_param_cfg_t *vencoder_tuning_param;
#endif
...
struct isp_dynamic_awb_item {
	HW_S16		value[ISP_AWB_MAX];
};

struct isp_dynamic_awb_cfg {
	HW_S16		trigger;
	struct isp_dynamic_awb_item tuning_cfg[ISP_DYNAMIC_GROUP_COUNT];
};
