...
#define MAX_AUDIOPCMFRAME_NODE_NUM (16)
typedef struct AudioFrameManager
{
    struct list_head mIdleList;    //AEncCompInputFrameNode
...
typedef struct AUDIOENCDATATYPE {
    ...
    AudioFrameManager   mBufQ; //used for managing input pcm data.
    struct list_head    mIdleOutFrameList;  //ENCODER_NODE_T, AEncCompOutputBuffer
    struct list_head    mReadyOutFrameList; //ENCODER_NODE_T, AEncCompOutputBuffer, for non-tunnel mode. when mOutputPortTunnelFlag == FALSE, use it to store encoded frames.
    struct list_head    mUsedOutFrameList; //ENCODER_NODE_T, AEncCompOutputBuffer, for non-tunnel mode. when mOutputPortTunnelFlag == FALSE, use it to store user occupied frames.
    int                 mFrameNodeNum;
    ...
    long long first_audio_pts;  //unit:us
    long long last_chk_time;
} AUDIOENCDATATYPE;
