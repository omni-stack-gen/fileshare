...
typedef struct VENC_ATTR_S
{
    ...
    unsigned int mVbrOptEnable;  /* 0: use old vbr bitrate control, 1: use new vbr bitrate control. */
}VENC_ATTR_S;
...
typedef enum {
    VencEncppSharp_Disable = -1,
    VencEncppSharp_FollowISPConfig = 0,
    VencEncppSharp_Enable,
}VencEncppSharpSettingE;
...
typedef struct VENC_ENCPP_ATTR_S
{
    ...
    VencEncppSharpSettingE eEncppSharpSetting;
...
{
    SEIDataType_ISP = 1<<0,
    SEIDataType_VIPP = 1<<1,
    SEIDataType_VENC = 1<<2,
}SEIDataType;
...
typedef struct
{
    BOOL bEnableIsp2Ve;
    BOOL bEnableVe2Isp;
    VI_DEV nVipp; //indicate frame source.
}VENC_IspVeLinkAttr;

typedef struct
{
    eCameraStatus eEnCameraMove;
} VENC_Isp2VeExtraParam;
