...
{
    AUDIO_SAMPLE_RATE_E enSamplerate;   /**< ai set, ao set, sample rate */
    AUDIO_BIT_WIDTH_E   enBitwidth;     /**< ai set, ao set, bitwidth */
    AIO_MODE_E          enWorkmode;     /* master or slave mode */
    AUDIO_SOUND_MODE_E  enSoundmode;    /**< ai set, ao set. mono or stereo. used by ai&ao, it is dst output attr of mpi_ai, input attr of mpi_ao */
    ...
    /**
      point num per frame (80/160/240/320/480/1024/2048), i.e. period_size,
        ...
        used by ai&ao. ai get. ao can't set now.
    */
    ...
    */
    unsigned int        mChnCnt; //ai set, ao set
    unsigned int        u32ClkSel;      /* 0: AI and AO clock is separate 1: AI and AO clock is inseparate, AI use AO's clock */
    PCM_CARD_TYPE_E     mPcmCardId;     /**< ao set, audio card selection for play audio file */
    int nAlsaFrameNumberForStartThreshold; // ao set. 0 means default value, one period in common.
    /**
    ...
    */
    int mMicNum; //ai set
    int ai_aec_en; //ai set
    /**
      when ref pcm delay(because rec pcm is processed by alsa automatic conversion plugin for det sampleRate and
      channels, this process will cost some time, so if we want capture pcm and ref pcm is in same time, we must discard
      some ref pcm data because they are previous time pcm data)
    */
    int aec_delay_ms;
    int mAecNlpMode; //0,1,2. kAecNlpConservative, kAecNlpModerate, kAecNlpAggressive
    /**
...
typedef struct AUDIO_FRAME_S
{
    ...
    unsigned int        nBufSize; //size of pAddr, extended for ao. unit: bytes
} AUDIO_FRAME_S;
...
{
    BOOL bUsrMode;          /* mode 0: auto???mode 1: manual.*/
...
typedef struct AI_VQE_CONFIG_S
{
    ...
    int              s32WorkSampleRate;  /* Sample Rate??8KHz/16KHz??default: 8KHz*/
...
/**
  ...
  @return
    ...
    -1:valid frame is not ready, process is failure.
*/
typedef int (*AgcProcessFuncType)(void *cookie, AUDIO_FRAME_S *pFrm);
/**
  get agclib inner data info.

  @return
    0: get data info success
    -1: get data info fail.
*/
typedef int (*AgcGetInnerDataInfoFuncType)(void *cookie, char **ppCacheProcessedData, int *pCacheProcessedDataLen,
